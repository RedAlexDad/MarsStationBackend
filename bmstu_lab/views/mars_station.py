...
def PUT_MarsStation_BY_USER(request, pk, format=None):
    ...
    mars_station.id_employee_id = request.data['id_employee']
...
def PUT_MarsStation_BY_ADMIN(request, pk, format=None):
    ...
    mars_station.id_moderator_id = request.data['id_moderator']
    ...
    if request.data['status_task'] in [3, 4]:
        ...
        return Response({'message': 'Вы можете обновить статус только с этими "Принята" или "Отменена"'}, status=status.HTTP_405_METHOD_NOT_ALLOWED)
